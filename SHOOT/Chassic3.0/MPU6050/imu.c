...
#ifdef NEW_FRICTION_POSITION
	MPU6050_Real_Data.Gyro_Yaw = mygetqval[3];	//三轴角速度
#else
	MPU6050_Real_Data.Gyro_Yaw = -mygetqval[3];
#endif
	MPU6050_Real_Data.Gyro_Pitch = -mygetqval[4];
	MPU6050_Real_Data.Gyro_Roll = -mygetqval[5];
...
//	yaw_angle = yaw_temp + yaw_count*360;  //yaw轴角度
  ...
  pitch_angle = -angle[2];	
	...
	if(Get_Time_Micros() > 1000000)	
	{
		yaw_angle += MPU6050_Real_Data.Gyro_Roll*(Get_Time_Micros() - time)/1000/1000;
